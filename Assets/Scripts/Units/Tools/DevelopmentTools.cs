...
namespace Units.Tools
{
    public class DevelopmentTools
    {
        ...
        /// <returns></returns>
        ///
        public static Vector2 To2DVector(Vector3 pos)
        {
            return new Vector2(pos.x, pos.z);
        }
        public static Vector3 WorldToScreenPoint(Vector3 worldPos)
        {
            RectTransform canvasRectTransform = SceneAssets.UiSubSystem.GetCanvasRectTransform(); 
            Vector2 screenPos = RectTransformUtility.WorldToScreenPoint(SceneAssets.CameraSubSystem.ActiveMainCamera, worldPos);

            RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRectTransform, screenPos, SceneAssets.CameraSubSystem.ActiveUICamera, out var position);

            return position;
        }
        public static Vector2 ScreenPointToUILocalPoint(RectTransform canvas, Vector2 point)
        {
            point.x = Mathf.Clamp(point.x, 0, Screen.width);
            point.y = Mathf.Clamp(point.y, 0, Screen.height);
            
            RectTransformUtility.ScreenPointToLocalPointInRectangle(canvas, point, SceneAssets.CameraSubSystem.ActiveUICamera, out Vector2 localPoint);
            return localPoint;
        }
